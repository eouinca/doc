...
class AppViewHolder(private val binding: AppListItemBinding) : BaseViewHolder<PackageInfo>(binding.root), View.OnClickListener {
    ...
    override fun onBind() {
        ...
        name.text = if (userId != UserHandleCompat.myUserId()) {
            val userInfo = docSystemApis.getUserInfo(userId)
        ...
        root.visibility = if (ai.metaData != null && ai.metaData.getBoolean("moe.doc.client.V3_REQUIRES_ROOT")) View.VISIBLE else View.GONE
