...
import moe.doc.manager.adb.AdbPairingTutorialActivity
import moe.doc.manager.databinding.HomeItemContainerBinding
import moe.doc.manager.databinding.HomeStartWirelessAdbBinding
import moe.doc.manager.ktx.toHtml
import moe.doc.manager.starter.StarterActivity
...
class StartWirelessAdbViewHolder(binding: HomeStartWirelessAdbBinding, root: View) :
    ...
    private fun onPairClicked(context: Context) {
        if ((context.display?.displayId ?: -1) > 0) {
            ...
            // pairing dialog can be displayed simultaneously with doc.
