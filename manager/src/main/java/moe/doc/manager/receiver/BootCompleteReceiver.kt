...
import moe.doc.manager.AppConstants
import moe.doc.manager.docSettings
import moe.doc.manager.docSettings.LaunchMethod
import moe.doc.manager.adb.AdbClient
import moe.doc.manager.adb.AdbKey
import moe.doc.manager.adb.AdbMdns
import moe.doc.manager.adb.PreferenceAdbKeyStore
import moe.doc.manager.starter.Starter
...
class BootCompleteReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (UserHandleCompat.myUserId() > 0 || doc.pingBinder()) return

        if (docSettings.getLastLaunchMode() == LaunchMethod.ROOT) {
            rootStart(context)
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU // https://r.android.com/2128832
            && context.checkSelfPermission(WRITE_SECURE_SETTINGS) == PackageManager.PERMISSION_GRANTED
            && docSettings.getLastLaunchMode() == LaunchMethod.ADB) {
            adbStart(context)
        } else {
    ...
    private fun adbStart(context: Context) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val adbMdns = AdbMdns(context, AdbMdns.TLS_CONNECT) { port ->
                ...
                try {
                    val keystore = PreferenceAdbKeyStore(docSettings.getPreferences())
                    val key = AdbKey(keystore, "doc")
