...
#define PACKAGE_NAME "moe.doc.privileged.api"
#define SERVER_NAME "doc_server"
#define SERVER_CLASS_PATH "rikka.doc.server.docService"
...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    ARG_PUSH_FMT(argv, "-Ddoc.library.path=%s", lib_path)
...
static void start_server(const char *path, const char *main_class, const char *process_name) {
    ...
    switch (pid) {
        ...
        default: {
            printf("info: doc_server pid is %d\n", pid);
            printf("info: doc_starter exit with 0\n");
            exit(EXIT_SUCCESS);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    uid_t uid = getuid();
    if (uid != 0 && uid != 2000) {
        perrorf("fatal: run doc from non root nor adb user (uid=%d).\n", uid);
        exit(EXIT_FATAL_UID);
    }
    ...
    foreach_proc([](pid_t pid) {
        ...
        if (kill(pid, SIGKILL) == 0)
            ...
            perrorf("fatal: can't kill %d, please try to stop existing doc from app first.\n", pid);
