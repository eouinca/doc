...
import static rikka.doc.docApiConstants.ATTACH_APPLICATION_API_VERSION;
import static rikka.doc.docApiConstants.ATTACH_APPLICATION_PACKAGE_NAME;
import static rikka.doc.docApiConstants.BIND_APPLICATION_PERMISSION_GRANTED;
import static rikka.doc.docApiConstants.BIND_APPLICATION_SERVER_PATCH_VERSION;
import static rikka.doc.docApiConstants.BIND_APPLICATION_SERVER_SECONTEXT;
import static rikka.doc.docApiConstants.BIND_APPLICATION_SERVER_UID;
import static rikka.doc.docApiConstants.BIND_APPLICATION_SERVER_VERSION;
import static rikka.doc.docApiConstants.BIND_APPLICATION_SHOULD_SHOW_REQUEST_PERMISSION_RATIONALE;
import static rikka.doc.docApiConstants.REQUEST_PERMISSION_REPLY_ALLOWED;
import static rikka.doc.docApiConstants.REQUEST_PERMISSION_REPLY_IS_ONETIME;
import static rikka.doc.server.ServerConstants.MANAGER_APPLICATION_ID;
import static rikka.doc.server.ServerConstants.PERMISSION;
...
import moe.doc.common.util.BuildUtils;
import moe.doc.common.util.OsUtils;
import moe.doc.server.IdocApplication;
...
import rikka.doc.server.api.IContentProviderUtils;
import rikka.doc.server.util.HandlerUtil;
import rikka.doc.server.util.UserHandleCompat;

public class docService extends Service<docUserServiceManager, docClientManager, docConfigManager> {

    public static void main(String[] args) {
        DdmHandleAppName.setAppName("doc_server", 0);
        RishConfig.setLibraryPath(System.getProperty("doc.library.path"));
    ...
    private final docClientManager clientManager;
    private final docConfigManager configManager;
    ...
    @Override
    public docUserServiceManager onCreateUserServiceManager() {
        return new docUserServiceManager();
    }

    @Override
    public docClientManager onCreateClientManager() {
        return new docClientManager(getConfigManager());
    }

    @Override
    public docConfigManager onCreateConfigManager() {
        return new docConfigManager();
    }
    ...
    @Override
    public void attachApplication(IdocApplication application, Bundle args) {
        ...
        int replyServerVersion = docApiConstants.SERVER_VERSION;
        if (apiVersion == -1) {
            // docBinderWrapper has adapted API v13 in dev.rikka.doc:api 12.2.0, however
        ...
        reply.putInt(BIND_APPLICATION_SERVER_PATCH_VERSION, docApiConstants.SERVER_PATCH_VERSION);
        if (!isManager) {
    ...
    private int  getFlagsForUidInternal(int uid, int mask, boolean allowRuntimePermission) {
        docConfig.PackageEntry entry = configManager.find(uid);
        if (entry != null) {
            ...
            for (PackageInfo pi : PackageManagerApis.getInstalledPackagesNoThrow(PackageManager.GET_META_DATA | PackageManager.GET_PERMISSIONS, user)) {
                ...
                if (flags != 0) {
                    list.add(pi);
                        ...
                        && pi.applicationInfo.metaData.getBoolean("moe.doc.client.V3_SUPPORT", false)
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        if (code == ServerConstants.BINDER_TRANSACTION_getApplications) {
            data.enforceInterface(docApiConstants.BINDER_DESCRIPTOR);
        ...
        try {
            ...
            if (!provider.asBinder().pingBinder()) {
                ...
                if (retry) {
                    ...
                    // Kill doc app and try again could work
            ...
            extra.putParcelable("moe.doc.privileged.api.intent.extra.BINDER", new BinderContainer(binder));
