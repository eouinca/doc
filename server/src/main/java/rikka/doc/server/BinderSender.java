...
import rikka.doc.server.util.Logger;

public class BinderSender {

    private static final Logger LOGGER = new Logger("BinderSender");

    private static final String PERMISSION_MANAGER = "moe.doc.manager.permission.MANAGER";
    private static final String PERMISSION = "moe.doc.manager.permission.API_V23";

    private static docService sdocService;
    ...
    private static void sendBinder(int uid, int pid) throws RemoteException {
        ...
        for (String packageName : packages) {
            ...
            if (ArraysKt.contains(pi.requestedPermissions, PERMISSION_MANAGER)) {
                ...
                if (granted) {
                    docService.sendBinderToManger(sdocService, userId);
                    return;
                }
            } else if (ArraysKt.contains(pi.requestedPermissions, PERMISSION)) {
                docService.sendBinderToUserApp(sdocService, packageName, userId);
                return;
            }
        }
    }

    public static void register(docService docService) {
        sdocService = docService;
